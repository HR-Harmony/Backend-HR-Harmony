...
func GetAnnouncementsByAdmin(db *gorm.DB, secretKey []byte) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		// Assign department names to announcements and update database
		tx := db.Begin()
		for i := range announcements {
			if departmentName, ok := departmentMap[announcements[i].DepartmentID]; ok {
				announcements[i].DepartmentName = departmentName
			}
			if err := tx.Save(&announcements[i]).Error; err != nil {
				tx.Rollback()
				errorResponse := helper.Response{Code: http.StatusInternalServerError, Error: true, Message: "Error saving announcement data"}
				return c.JSON(http.StatusInternalServerError, errorResponse)
			}
		}
		tx.Commit()
