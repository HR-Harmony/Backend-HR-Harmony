...
func GetAllDepartmentsByAdmin(db *gorm.DB, secretKey []byte) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		// Assign full names to departments and update database
		tx := db.Begin()
		for i := range departments {
			if fullName, ok := employeeMap[departments[i].EmployeeID]; ok {
				departments[i].FullName = fullName
			}
			if err := tx.Save(&departments[i]).Error; err != nil {
				tx.Rollback()
				errorResponse := helper.Response{Code: http.StatusInternalServerError, Error: true, Message: "Error saving department data"}
				return c.JSON(http.StatusInternalServerError, errorResponse)
			}
		}
		tx.Commit()
